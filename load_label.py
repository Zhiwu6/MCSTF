...
def load_excel(dataset_name):
    if (dataset_name == 'CASME_sq'):
        ...
        print("Loading Excel结束")

    elif (dataset_name == 'SAMMLV'):
        xl = pd.ExcelFile(dataset_name + '/SAMM_LongVideos_V3_Release.xlsx')

        colsName = ['Subject', 'Filename', 'Inducement Code', 'Onset', 'Apex', 'Offset', 'Duration', 'Type',
                    'Action Units', 'Notes']
            ...
            for i, row in codeFinal.iterrows():
                if (row['subjectCode'] == subjects[
                    ...
                    if (row['videoCode'] == videoCode):
                        ...
                        if (row['type'] == dataset_expression_type):
                            # Micro-expression or macro-expression
