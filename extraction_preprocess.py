...
def extract_preprocess(final_images, k):
    ...
    for video in range(len(final_images)):
        ...
        for img_count in range(final_images[video].shape[0] - k):
            ...
            # img3 = final_images[video][img_count + k - 1]
            ...
            # 可视化 final_image
            # plt.imshow(final_image, cmap='jet')  # 根据需要选择颜色映射
            # plt.colorbar()  # 添加颜色条
            # plt.axis('off')  # 关闭坐标轴
            # plt.show()
            # 可视化第一个维度
            # plt.imshow(final_image[:, :, 2], cmap='gray')  # 选择第一个维度
            # plt.colorbar()  # 添加颜色条
            # plt.axis('off')  # 关闭坐标轴
            # plt.show()
            # 分别可视化三个通道
            ...
            # # 可视化第二个通道
            # plt.subplot(1, 3, 2)
            # plt.imshow(final_image[:, :, 1], cmap='gray')
            # plt.colorbar()
            # plt.title('Channel 2')
            # # 可视化第三个通道
            # plt.subplot(1, 3, 3)
            # plt.imshow(final_image[:, :, 2], cmap='gray')
            # plt.colorbar()
            # plt.title('Channel 3')
            # plt.tight_layout()
            # plt.show()
